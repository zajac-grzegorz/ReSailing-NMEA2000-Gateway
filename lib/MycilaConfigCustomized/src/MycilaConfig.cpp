...
void Mycila::Config::configure(const char* key, const char* defaultValue) {
  ...
  _keys.push_back(key);
  std::sort(_keys.begin(), _keys.end(), [](std::string a, std::string b) { return a.compare(b) < 0; });
  if (defaultValue)
...
bool Mycila::Config::set(const std::map<const char*, std::string>& settings, bool fireChangeCallback) {
  ...
  for (auto& key : _keys)
    if (!isEnableKey(key.c_str()) && settings.find(key.c_str()) != settings.end())
      updates |= set(key.c_str(), settings.at(key.c_str()).c_str(), fireChangeCallback);
...
bool Mycila::Config::restore(const char* data) {
  ...
  for (auto& key : _keys) {
    ...
    char* start = strstr(data, key.c_str());
    if (start) {
      start += strlen(key.c_str());
      if (*start != '=')
        continue;
      start++;
      ...
      settings[key.c_str()] = std::string(start, end - start);
    }
  }
...
void Mycila::Config::toJson(const JsonObject& root) {
  for (auto& key : _keys) {
    const std::string& value = get(key.c_str());
  #ifdef MYCILA_CONFIG_PASSWORD_MASK
    root[key] = value.empty() || !isPasswordKey(key.c_str()) ? value : MYCILA_CONFIG_PASSWORD_MASK;
  #else
    root[key] = value;
